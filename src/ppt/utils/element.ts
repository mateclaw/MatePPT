...
import { type PPTElement, type LineElement, type PPTSlide, PPTElementType } from '@/ppt/core'
...
export const getElementRange = (element: PPTElement) => {
  ...
  if (element.type === 'line') {
    ...
    minX = lineElement.left
    maxX = lineElement.left + Math.max(lineElement.start?.[0] || 0, lineElement.end?.[0] || 0)
    minY = lineElement.top
    maxY = lineElement.top + Math.max(lineElement.start?.[1] || 0, lineElement.end?.[1] || 0)
  }
  else if ('rotate' in element && element.rotate) {
    const { left, top, width, height, rotate } = element
    const { xRange, yRange } = getRectRotatedRange({ left: left, top: top, width: width, height: height, rotate })
    minX = xRange[0]
    maxX = xRange[1]
    minY = yRange[0]
    maxY = yRange[1]
 ...
 * @param element 线条元素
 */
export const getLineElementLength = (element: LineElement) => {
 ...
 * @param slides 页面列表
 */
export const createSlideIdMap = (slides: PPTSlide[]) => {
 ...
 * @param element 线条元素
 */
export const getLineElementPath = (element: LineElement) => {
