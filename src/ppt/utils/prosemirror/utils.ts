...
type Align = 'left' | 'right' | 'center' | 'justify' | 'distributed'
...
export const getTextAttrs = (view: EditorView, attrs: Partial<DefaultAttrs> = {}) => {
  ...
  const colorScheme = getAttrValue(marks, 'forecolor', 'scheme') || defaultAttrs.colorScheme
  const colorTransforms = getAttrValue(marks, 'forecolor', 'transforms') || defaultAttrs.colorTransforms
