...
const forecolor: MarkSpec = {
  ...
  parseDOM: [
    ...
    {
      tag: 'span[data-scheme]',
      getAttrs: (dom) => {
        const el = dom as HTMLElement
        const color = el.style?.color || null
        if (!color) return false
        const scheme = el.getAttribute('data-scheme') || null
        const transforms = el.getAttribute('data-transforms') || null
        return { color, scheme, transforms }
      },
    },
  ],
  toDOM: mark => {
    const { color, scheme, transforms } = mark.attrs as { color?: string; scheme?: string | null; transforms?: string | null }
    let style = ''
    if (color) style += `color: ${color};`
    const attrs: Record<string, string> = { style }
    if (scheme) attrs['data-scheme'] = scheme
    if (transforms) attrs['data-transforms'] = transforms
    return ['span', attrs, 0]
  },
}
