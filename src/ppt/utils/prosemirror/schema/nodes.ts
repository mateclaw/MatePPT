  ...
  parseDOM: [
    {
      tag: 'p',
      getAttrs: dom => {
        const { textAlign, textIndent,  textAlignLast } = (dom as HTMLElement).style

        let align = (dom as HTMLElement).getAttribute('align') || textAlign || ''
        if (!align && (textAlign && /distribute/.test(textAlign) || textAlignLast === 'justify')) {
          align = 'distributed'
        }
        else if (align === 'justify' && (textAlign && /distribute/.test(textAlign) || textAlignLast === 'justify')) {
          align = 'distributed'
        }
        align = /(left|right|center|justify|distributed)/.test(align) ? align : ''
