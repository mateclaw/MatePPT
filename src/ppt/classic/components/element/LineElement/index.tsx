...
import type { LinePoint, PPTLineElement } from '@/ppt/core'
...
import styles from './LineElement.module.scss'
...
const LineElement: FC<LineElementProps> = ({
  elementInfo,
  selectElement,
  contextmenus,
}) => {
  ...
  return (
    <>
      <div
        className={clsx(
          styles.editableElementLine,
          elementInfo.lock && styles.lock,
        )}
        style={containerStyle}
        data-line-id={elementInfo.id}
      >
          ...
          >
            <svg overflow="visible" width={svgWidth} height={svgHeight}>
              <defs>
                {safePoints[0] && (
                  <LinePointMarker
                    id={elementInfo.id}
                    position="start"
                    type={safePoints[0] as Exclude<LinePoint, ''>}
                    color={lineColor}
                ...
                {safePoints[1] && (
                  <LinePointMarker
                    id={elementInfo.id}
                    position="end"
                    type={safePoints[1] as Exclude<LinePoint, ''>}
                    color={lineColor}
                    baseSize={elementInfo.strokeWidth ?? elementInfo.width ?? 1}
                  />
                )}
              ...
              <path
                ...
                markerStart={
                  safePoints[0]
                    ? `url(#${elementInfo.id}-${safePoints[0]}-start)`
                    : undefined
                }
                markerEnd={
                  safePoints[1]
                    ? `url(#${elementInfo.id}-${safePoints[1]}-end)`
                    : undefined
                }
              />
              <path
      ...
      {contextmenuAxis && menus.length > 0 && (
        <Contextmenu
          ...
          el={document.querySelector(`[data-line-id="${elementInfo.id}"]`) || document.body}
          menus={menus}
