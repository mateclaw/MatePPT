...
export interface PasteTextClipboardDataOptions {
  /** 仅处理页面粘贴 */
  onlySlide?: boolean
  /** 仅处理元素粘贴 */
...
const isValidImgURL = (url: string) => {
  const pexels = /^https?:\/\/(?:[a-zA-Z0-9-]+\.)*pexels\.com\/[^\s]+\.(?:jpg|jpeg|png|svg|webp)(?:\?.*)?$/i.test(
    url,
  )
  const pptist =
...
/**
 * 解析剪贴板文本内容，根据内容类型：
 * - 自定义 elements/slides JSON：调用 addElementsFromData / addSlidesFromData
 * - 普通文本：创建文本元素 / 图片 / 链接 / SVG 图片
 */
export const usePasteTextClipboardData = () => {
  ...
  const pasteTextClipboardData = (
    ...
    if (typeof clipboardData === 'object') {
      const { type, data } = clipboardData as {
        type: 'elements' | 'slides'
        data: any
      }
...
export default usePasteTextClipboardData
